...
RESULTS_FILE = "grid_results_select_{}.csv".format(time.strftime("%m-%d_%H"))
NUM_CORES = 8
...
class BatchAssessment(BaseModel):
    InterestingImages: list[SelectedImage] = Field(
        description="List of selected images from the grid. Can be empty if none are interesting."
    )
...
config = types.GenerateContentConfig(
    system_instruction=(
        "You are an expert astronomer. You are viewing a 3x3 grid of astronomical images. "
...
def process_batches(args):
    ...
    with open(checkpoint_file, 'a', newline='', encoding='utf-8') as f:
        ...
        if os.stat(checkpoint_file).st_size == 0:
            writer.writerow(["Filename", "Reasoning"])
        ...
        for batch_images in batches:
            ...
            try:
                ...
                contents = [
                    ...
                    *artifact_files,
                    "Now analyze this 3x3 grid of astronomical images. Identify any that are scientifically interesting.",
                    uploaded_grid
                ]
                ...
                print(f"Worker {worker_id} processed a batch. Found {len(data.InterestingImages)} interesting images.")

                for selection in data.InterestingImages:
                    idx = selection.GridIndex - 1
                    if 0 <= idx < len(batch_images):
                        orig_fname = batch_images[idx]
                        print(f"Worker {worker_id} SELECTED: {orig_fname}")
                        writer.writerow([orig_fname, selection.Reasoning])
    ...
    for i in range(NUM_CORES):
        ...
        if os.path.exists(cp):
            with open(cp, 'r') as f:
                ...
                next(reader)
                for row in reader:
                    if row and row[0] in scores:
                        scores[row[0]] += 1
                        if not reasons[row[0]]: reasons[row[0]] = row[1]
            os.remove(cp)
