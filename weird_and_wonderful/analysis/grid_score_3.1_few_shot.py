...
The model is prompted with representative artifact, interesting, and boring/high-score examples. These examples are not exhaustive.
'''
import os
import csv
import time
...
RESULTS_FILE = "grid_results_score_3.1_low_4x4_fewshot_{}.csv".format(time.strftime("%m-%d_%H"))
NUM_CORES = 8
...
INTERESTING_EXAMPLES = [
    "70399534798243618.png",
    "69568737799335512.png",
    "40132479356530446.png",
    "70409168409886037.png",
    "41232532150183712.png",
]

BORING_HIGH_SCORE_EXAMPLES = [
    "40171920041206815.png",
    "40155152488881073.png",
    "70417423337023796.png",
    "69559525094478257.png",
]
...
def process_batches(args):
    worker_id, batches, api_key, example_uris, progress_queue = args
    client = genai.Client(api_key=api_key)
    checkpoint_file = f"checkpoint_grid_worker_{worker_id}.csv"

    # Re-hydrate example file references from URIs for use in contents
    artifact_files = [types.File(uri=uri, mime_type="image/png") for uri in example_uris["artifacts"]]
    interesting_files = [types.File(uri=uri, mime_type="image/png") for uri in example_uris["interesting"]]
    boring_high_score_files = [types.File(uri=uri, mime_type="image/png") for uri in example_uris["boring_high_score"]]
    ...
    with open(checkpoint_file, 'a', newline='', encoding='utf-8') as f:
        ...
        for batch_images in batches:
            ...
            try:
                ...
                # Prepend representative examples to every prompt so the model has
                # visual references before seeing the grid. These examples are
                # representative, not exhaustive.
                contents = [
                    (
                        "The following examples are representative guidance images. "
                        "They are NOT exhaustive; use them to calibrate your judgment, "
                        "but still evaluate each new grid image independently."
                    ),
                    "Examples of ARTIFACTS that must NOT be selected:",
                    *artifact_files,
                    (
                        "Examples of SCIENTIFICALLY INTERESTING images that should be selected "
                        "when similar morphology appears:"
                    ),
                    *interesting_files,
                    (
                        "Examples of BORING / NOT SCIENTIFICALLY INTERESTING images."
                        " Do NOT select images merely because they look clean, bright, or centered:"
                    ),
                    *boring_high_score_files,
                    (
                        "Now analyze this 4x4 grid of astronomical images. "
                        "Select any scientifically interesting images, rate each 1-5. "
                        "Remember: the examples above are representative, not exhaustive."
                    ),
                    uploaded_grid
                ]
...
if __name__ == '__main__':
    ...
    # Upload representative examples once at startup; pass URIs to workers so each
    # worker can reference them without re-uploading
    print("Uploading representative examples...")
    client_main = genai.Client(api_key=api_key)

    def upload_examples(label, filenames):
        uris = []
        print(f"Uploading {label} examples...")
        for fname in filenames:
            path = os.path.join(IMAGE_DIR, fname)
            uploaded = client_main.files.upload(file=path)
            uris.append(uploaded.uri)
            print(f"  Uploaded {label}: {fname}")
        return uris

    example_uris = {
        "artifacts": upload_examples("artifact", ARTIFACT_EXAMPLES),
        "interesting": upload_examples("interesting", INTERESTING_EXAMPLES),
        "boring_high_score": upload_examples("boring/high-score", BORING_HIGH_SCORE_EXAMPLES),
    }
    print(
        "Done. "
        f"{len(example_uris['artifacts'])} artifact, "
        f"{len(example_uris['interesting'])} interesting, and "
        f"{len(example_uris['boring_high_score'])} boring/high-score examples ready.\n"
    )
    ...
    print(f"Total 2x2 Grids:   {len(all_batches)}")
    ...
    for i in range(NUM_CORES):
        ...
        if start < len(all_batches):
            worker_args.append((i+1, all_batches[start:end], api_key, example_uris, progress_queue))
