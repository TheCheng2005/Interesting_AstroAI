...
RESULTS_FILE = "grid_results_no_explain_{}.csv".format(time.strftime("%m-%d_%H"))
NUM_CORES = 8
...
config = types.GenerateContentConfig(
    system_instruction=(
        "You are an expert astronomer evaluating a 3x3 grid of astronomical images. "
        ...
        "Return ONLY a JSON list of selected images with GridIndex and Score. "
        "Do not include explanations, reasoning, comments, or any extra fields. "
        "If none are interesting, return an empty list.\n"
        f"{few_shot_context}"
    ),
                ...
                contents = [
                    ...
                    *artifact_files,
                    (
                        ...
                        "Return only GridIndex and Score; do not explain. "
                    ),
                    uploaded_grid
                ]
                ...
                if data:
                    for item in data:
                        ...
                        if 0 <= idx < len(batch_images) and batch_images[idx] is not None:
                            ...
                            writer.writerow([filename, item.Score])
...
if __name__ == '__main__':
    ...
    with open(RESULTS_FILE, 'w', newline='', encoding='utf-8') as f:
        ...
        for img in sorted(scores, key=scores.get, reverse=True):
            ...
            writer.writerow([
                img, score, "No reasoning available",
