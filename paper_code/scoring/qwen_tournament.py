...
- Qwen may keep any number of the four images (zero through four).
...
def results_filename(run_idx, seed):
    ...
    qwen_tournament_<run_idx>.csv; run_idx is the 1-based replicate number."""
    if TEST_MODE:
        os.makedirs(SUBSET_DIR, exist_ok=True)
        return os.path.join(SUBSET_DIR, f"qwen_tournament_{run_idx}.csv")
    return os.path.join(FULL_RUN_DIR, "qwen_tournament_seed{}_{}.csv".format(seed, time.strftime("%m-%d_%H")))
