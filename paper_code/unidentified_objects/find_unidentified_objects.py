...
def load_likert_run(path):
    """
    Load one scoring CSV via the shared reader.

    The reader accepts both header spellings the protocols emit and raises on
    one it cannot parse. That matters here: this function used to look up
    "filename" directly, so a Tournament CSV (which spells it "Filename")
    produced zero rows with no error at all, and the whole cross-match then
    ran on an empty catalogue and wrote empty outputs.
    """
    return [
        {
            "filename": row["filename"],
            "imagescore": int(float(row["imagescore"])),
            "SourceRA": float(row["SourceRA"]) if row["SourceRA"] else None,
            "SourceDec": float(row["SourceDec"]) if row["SourceDec"] else None,
        }
        for row in scores_csv.read(path).rows
    ]
