"""
Pairwise elimination tournament for astronomical image scoring (Qwen No-Shot).
...
- Qwen selects the more interesting image (without generating reasoning).
...
(Reasoning is written as an empty string to preserve schema)
"""
...
RESULT_TEMPLATE = os.path.join(RESULTS_DIR, "qwen_single_elimination_no_shot_990_run{}.csv")
NUM_CORES = 20
...
# No-Shot only uses Artifacts to know what NOT to select
ARTIFACT_EXAMPLES = [
    ...
    with open(checkpoint_file, "a", newline="", encoding="utf-8") as f:
        ...
        for match_number, pair_images in matches:
            ...
            try:
                ...
                writer.writerow([
                    round_number, match_number, winner, 1, "", loser, input_tok, output_tok, total_tok
                ])
...
def run_experiment(run_idx, api_key, all_images, artifact_b64s):
    ...
    while len(active_images) > 1:
        ...
        print(f"\n=== Round {round_number} | Entering: {len(active_images)} | Matches: {len(all_matches)} ===")
        ...
        for i in range(NUM_CORES):
            ...
            if start < len(all_matches):
                worker_args.append((
                    i + 1, round_number, all_matches[start:end], api_key, artifact_b64s, progress_queue
                ))
