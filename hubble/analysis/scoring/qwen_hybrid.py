...
using Qwen (via OpenRouter) instead of Gemini.

This combines the two existing formats to save tokens:

Phase 1 (tournament filter, qwen_tournament.py format):
- All active images are shuffled and grouped into 2x2 grids of four.
- Qwen sees a 2x2 grid labeled Image 1 (top-left), Image 2 (top-right),
...
def results_filename(run_idx, seed):
    ...
    qwen_hybrid_<run_idx>.csv; run_idx is the 1-based replicate number."""
    if TEST_MODE:
        os.makedirs(SUBSET_DIR, exist_ok=True)
        return os.path.join(SUBSET_DIR, f"qwen_hybrid_{run_idx}.csv")
    return os.path.join(FULL_RUN_DIR, "qwen_hybrid_seed{}_{}.csv".format(seed, time.strftime("%m-%d_%H")))


# ── Phase 1 (tournament filter) schema and prompt ───────────────────────────
...
# ── Phase 2 (Likert scoring) schema and prompt ──────────────────────────────
...
def process_matches(args):
    ...
    with open(checkpoint_file, "a", newline="", encoding="utf-8") as f:
        ...
        for group_number, group_records in groups:
            ...
            try:
                quad_b64 = base64.b64encode(quad_bytes).decode()

                response = retry_api(
                    client.beta.chat.completions.parse,
                    model=OPENROUTER_MODEL,
                    messages=[
                        {"role": "system", "content": TOURNAMENT_SYSTEM_INSTRUCTION},
                        {"role": "user", "content": [
                            {
                                "type": "text",
                                "text": (
                                    ...
                                    "Evaluate each image independently. Do not force a winner. "
                                    "For each image, decide whether it is scientifically "
                                    "interesting enough to keep for human inspection."
                                ),
                            },
                            {
                                "type": "image_url",
                                "image_url": {
                                    "url": f"data:image/png;base64,{quad_b64}"
                                },
                            },
                        ]},
                    ],
                ...
                # If Qwen/API fails, keep every image in the group so we do not
        ...
        for batch_records in batches:
            ...
            try:
                ...
                response = retry_api(
                    ...
                    model=OPENROUTER_MODEL,
                    messages=[
                        {"role": "system", "content": LIKERT_SYSTEM_INSTRUCTION},
                        {"role": "user", "content": [
                            {
                                "type": "text",
                                "text": (
...
def run_tournament_filter(active_records, api_key, random_seed):
    ...
    Returns (survivors, eliminated, input_tokens, output_tokens, cost), where
    ...
    return survivors, eliminated, round_input_tokens, round_output_tokens, round_cost
...
def run_likert_scoring(survivors, api_key, random_seed):
    ...
    scored 1-5 per selection. Returns (scores, input_tokens, output_tokens, cost),
    ...
    return scores, total_prompt_tokens, total_candidate_tokens, total_cost
...
def run_experiment(all_images, api_key, random_seed, results_file):
    ...
    survivors, eliminated, tourn_in_tok, tourn_out_tok, tourn_cost = run_tournament_filter(
        all_images[:], api_key, random_seed
    )

    # ── Phase 2: Likert scoring of survivors ───────────────────────────────
    survivor_scores, likert_in_tok, likert_out_tok, likert_cost = run_likert_scoring(
    ...
    total_cost = tourn_cost + likert_cost
