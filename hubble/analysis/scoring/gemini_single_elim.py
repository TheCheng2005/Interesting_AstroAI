"""
Pairwise elimination tournament for Hubble image scoring.
...
- Gemini selects the more interesting image (no reasoning requested, to save tokens).
...
def results_filename(run_idx, seed):
    ...
    gemini_single_elim_<run_idx>.csv; run_idx is the 1-based replicate number."""
    if TEST_MODE:
        os.makedirs(SUBSET_DIR, exist_ok=True)
        return os.path.join(SUBSET_DIR, f"gemini_single_elim_{run_idx}.csv")
    return os.path.join(FULL_RUN_DIR, "single_elim_seed{}_{}.csv".format(seed, time.strftime("%m-%d_%H")))
...
def process_matches(args):
    ...
    with open(checkpoint_file, "a", newline="", encoding="utf-8") as f:
        ...
        for match_number, pair_records in matches:
            ...
            try:
                pair_part = types.Part.from_bytes(
                    data=pair_bytes,
                    mime_type="image/png",
                )
