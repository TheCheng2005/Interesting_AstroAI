"""
I ask Gemini to pick any number of interesting images from a 4x4 grid.
Gemini scores each selected image on a scale of 1-5.
...
- Final test set (TEST_MODE):
      Every interesting image is injected manually, and the remainder of the
      TEST_NUM_IMAGES budget is filled with a random subset of boring images.
      Outside TEST_MODE, ALL images from the HDF5 file are used.

- The experiment is run once per seed in RANDOM_SEEDS, producing one CSV
  file per seed.
...
def results_filename(run_idx, seed):
    ...
    gemini_likert_<run_idx>.csv; run_idx is the 1-based replicate number."""
    if TEST_MODE:
        os.makedirs(SUBSET_DIR, exist_ok=True)
        return os.path.join(SUBSET_DIR, f"gemini_likert_{run_idx}.csv")
    return os.path.join(FULL_RUN_DIR, "10M_10arcsec_dedup_seed{}_{}.csv".format(seed, time.strftime("%m-%d_%H")))
