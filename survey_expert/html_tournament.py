...
------------------------------
Reads one AI classification CSV plus any number of expert CSV files and writes a
self-contained HTML report comparing AI classifications, expert selections, and
volunteer ratings.

Expected AI CSV columns, flexible casing supported:
...
Usage:
    python generate_classfication_html.py tournament_filter_05-24_15.csv
    python generate_classfication_html.py tournament_filter_05-24_15.csv ./expert_csvs
"""
    ...
    try:
        with open(path, newline="", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                # The tournament CSV is the authoritative image whitelist.
                # Ignore expert-survey entries for images not present there.
                if filename not in ai_rows:
                    continue
...
all_filenames = sorted(ai_rows.keys())
...
for filename in all_filenames:
    ...
    if ai is None:
        ...
        ai = {
            ...
            "ai_selected": 0,
            "ai_interest": "No",
            "reasoning": "Not present in AI CSV.",
            "ra": "N/A",
            "dec": "N/A",
            "anomaly_score": "N/A",
...
<div class="page">
  ...
  <footer>
    Expert files: {esc(", ".join(os.path.basename(p) for p in expert_files))}<br>
    Missing from AI CSV: {len(missing_from_ai)} · Missing from expert CSVs: {len(missing_from_experts)}
  </footer>
